...
public class EFS extends Utility {
    ...
    public byte[] read(String file_name, int starting_position, int len, String password) throws Exception {
        ...
        // Read boundaries
        if (starting_position < 0 || starting_position >= meta.file_length || len < 0
                || starting_position + len > meta.file_length) {
    ...
    private Metadata validatePwd(String file_name, String password) throws Exception {
        ...
        byte[] encryptedKeys = Arrays.copyOfRange(metadata, USERNAME_MAX + SALT_SIZE,
                USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE);
        byte[] nonce = Arrays.copyOfRange(metadata, USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE,
                USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE + NONCE_SIZE);
        int file_length = ByteBuffer
                .wrap(Arrays.copyOfRange(metadata, USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE + NONCE_SIZE,
                        USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE + NONCE_SIZE + 4))
                .getInt();
        byte[] storedMac = Arrays.copyOfRange(metadata, USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE + NONCE_SIZE + 4,
                USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE + NONCE_SIZE + 4 + MAC_SIZE);
    ...
    private void updateMetadata(String file_name, String password, int new_length, Metadata meta) throws Exception {
        byte[] metadata = Files.readAllBytes(Paths.get(file_name, "0"));
        System.arraycopy(intToBytes(new_length), 0, metadata,
                USERNAME_MAX + SALT_SIZE + FEK_SIZE + MK_SIZE + NONCE_SIZE, 4);
